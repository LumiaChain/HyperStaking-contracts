...
import '@openzeppelin-v4.9.5/contracts/security/ReentrancyGuard.sol';
...
contract VaultFactory is ReentrancyGuard, VaultFactoryConfig, VaultFactoryList {
    ...
    event vaultLiquidated(
        address vault,
        uint256 forgivenDebt,
        address[] liquidatedCollaterals,
        uint256[] _liquidatedAmounts
    );
     ...
     * @param _borrowFeeRecipient Address receiver for the fees generated when borrowing.
     * @param _redemptionFeeRecipient Address receiver for the fees generated when redeeming.
     */
    constructor(
    ...
    function liquidate(address _vault) external nonReentrant {
        ...
        ) = Vault(_vault).liquidate();

        for (uint i; i < _liquidatedCollaterals.length; ) {
            collateral[_liquidatedCollaterals[i]] -= _liquidatedAmounts[i];
            unchecked {
                i++;
            }
        }
