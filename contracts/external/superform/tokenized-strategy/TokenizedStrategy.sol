...
contract TokenizedStrategy {
    ...
    function simulateYieldGeneration(address singleVault, uint256 yieldAmount) external onlyManagement {
        StrategyData storage S = _strategyStorage();

        // Simulate yield by increasing the strategy's asset balance
        S.asset.safeTransferFrom(msg.sender, singleVault, yieldAmount);

        S.totalAssets += yieldAmount;
    }
