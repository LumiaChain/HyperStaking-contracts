...
describe("Aera", function () {
  ...
  describe("Gauntlet Strategy", function () {
    ...
    it("compounds before user redeem, user claims more tokens than initial stake", async function () {
      ...
      let expectedExitShares = await gauntletStrategy.previewAllocation(exitAmount);
  ...
  describe("GauntletStrategy gain/loss fuzz", () => {
    it("stake/redeem across slippage, price moves, and redeem ratios", async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.record({
            // 100 .. 100_000 USDC
